...
'''
Title: unioncount_reg_fracMetCs.py
Date: 20191012
...
Description:
	This script iterates over a positions file generated by bedtools unionbedg with "methylated/total" read counts (unioncount file)
	and a corresponding regions file, to give the fraction of methylated cytosines as "methylated/total" cytosines
	found (see Schultz et al. 2013) within each region.
	NB: Prior to this script, positions in the unionbed file should be tested for having methylation higher than the
	error rate (non conversion rate) through a binomial test. methylated count for positionf failing the test should be converted to 0.
	Cytosines are considered methylated if they have at least 1 read supporting methylation.
	It only works with position sorted bed files with distinct, non-overlapping regions. 
List of functions:
	main()
	buildIndex()
	moveToNextRegion()
...
Input:
	...
	2) simil-unionbedg file harbouring methylated/total calls for each position. NAs are allowed and unionbed should have headers
	3) Index file: a txt file with one column, containing all possible scaffolds (sorted) from both files.
Usage:
	unioncount_reg_fracMetCs.py [REGIONS] [POSITIONS] [INDEX] [FOUT]
eg. unioncount_reg_fracMetCs.py regions.bed positions.bed index.txt reg_meth.bed
'''
	...
	with open(REGIONS,'r') as regions, open(POSITIONS,'r') as positions, open(FOUT, 'w') as fout:
		...
		for position in positions:
			...
			if Found and ((current_Chr != Chr) or ((current_Chr == Chr) and (current_Pos >= End))):
				...
				#averages = [str(format(100*n[0]/n[1], '.4f')) if n[1] != 0 else "NA" for n in totals]	# To print average or ratio
				averages = [str(str(n[0])+"/"+str(n[1])) if n[1] != 0 else "NA" for n in totals]		# To print "meth/total"
			...
			# For weighted average methylation	
				#totals = [(totals[i][0],totals[i][1]) if position[i+3] == "NA" else (totals[i][0]+int(position[i+3].split("/")[0]),totals[i][1]+int(position[i+3].split("/")[1])) for i in range(0, length)]
			# For fraction of methylated Cs
				for i in range(0, length):
					if position[i+3] == "NA":
						totals[i] = (totals[i][0],totals[i][1])
					else:
						met=int(position[i+3].split("/")[0])
						#tot=int(position[i+3].split("/")[1])														# For weighted average methylation
						#totals = (totals[i][0]+met,totals[i][1]+tot)  												# For weighted average methylation
						totals[i] = ((totals[i][0]+1,totals[i][1]+1) if met > 0 else (totals[i][0],totals[i][1]+1))	# For fraction of methylated Cs	

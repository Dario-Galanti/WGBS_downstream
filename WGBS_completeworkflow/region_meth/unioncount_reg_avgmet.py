...
'''
Title: unioncount_reg_avgmet.py
Date: 20191012
...
Description:
	This script iterates over a positions file generated by bedtools unionbedg with "meth/total" read counts per position
	(unioncount file) and a corresponding regions file, to give the average methylation ('mean methylation' in Schultz et al. 2013)
	values of all positions contained within each region. It only works with position sorted bed files with distinct,
	non-overlapping regions, such as the DMRs from Metilene.
List of functions:
	main()
	buildIndex()
	moveToNextRegion()
...
Input:
	...
	2) simil-unionbedg file (unioncount) harbouring methylated/total calls for each position. NAs are allowed and unionbed should have headers
	3) Index file: a txt file with one column, containing all possible scaffolds (sorted) from both files.
Usage:
	unioncount_reg_avgmet.py [REGIONS] [POSITIONS] [INDEX] [FOUT]
eg. unioncount_reg_avgmet.py regions.bed positions.bed index.txt reg_meth.bed
'''
	...
	with open(REGIONS,'r') as regions, open(POSITIONS,'r') as positions, open(FOUT, 'w') as fout:
		...
		for position in positions:
			...
			if Found and ((current_Chr != Chr) or ((current_Chr == Chr) and (current_Pos >= End))):
				...
				averages = [str(format(100*n[0]/n[1], '.4f')) if n[1] != 0 else "NA" for n in totals]
				#print(totals)
				#print(averages)
			...
			## For average methylation
				totals = [(totals[i][0],totals[i][1]) if position[i+3] == "NA" else (totals[i][0]+float(int(position[i+3].split("/")[0])/int(position[i+3].split("/")[1])),totals[i][1]+1) for i in range(0, length)]
				Found = True
